...
async function deleteAccount(req, res) {
    ...
    const account = await User.deleteAccount(req.params.id, req.mongooseSession);
    if (!account) throw new TransactionError({status: 404, msg: 'Failed to delete account. Account not found.'});
    ...
    res.json({status: 200, msg: 'User account deleted successfully.', userId: account._id});
}
...
async function changePwd(req, res) {
    ...
    const data = await User.changePwdByAdmin(req.params.id, req.data.pwd, req.mongooseSession);
    if (!data) throw new TransactionError({status: 404, msg: 'Failed to change user password. User not found.'});

    await UserToken.clearToken(req.params.id, req.mongooseSession);

    res.json({status: 200, msg: 'User password changed successfully.', data});
}
...
async function blacklist(req, res) {
    ...
    const data = await User.blacklist(req.params.id, req.data.reason, req.userId, req.mongooseSession);
    if (!data) throw new TransactionError({status: 404, msg: 'Failed to blacklist user. User not found.'});

    await UserToken.clearToken(req.params.id, req.mongooseSession);

    res.json({status: 200, msg: 'User blacklisted successfully.', data});
}
